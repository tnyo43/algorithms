...
template<class T = int> struct LowestCommonAncestor {
private:
    int V;
    int logV;
    int root;
    ...
    void init(const vector<vector<pair<int, T>>> &g, int r, QUERY_GET qtype) {
        G = g;
        V = g.size();
        logV = log2(V)+1;
        depth = vector<int>(V);
...
public:
    ...
    LowestCommonAncestor(const vector<vector<int>> &g, int root = 0) {
        ...
        REP (i, g.size()) {
            h[i] = vector<pair<int, T>>(g[i].size());
            REP (j, g[i].size()) h[i][j] = make_pair(g[i][j], T(0));
        }
        init(h, root, DEFAULT);
    }
    LowestCommonAncestor(const vector<vector<pair<int, T>>> &g, int root = 0, QUERY_GET qtype = DEFAULT) { init(g, root, qtype); }
